...
public class FreeCamera : MonoBehaviour {
	...
	void Update() {
		...
		rb.MoveRotation(Quaternion.AngleAxis(m_yaw, Vector3.up) * Quaternion.AngleAxis(m_pitch, Vector3.right));
		...
		var up = .5f * speed * ((Input.GetKey(KeyCode.E) ? 1f : 0f) - (Input.GetKey(KeyCode.Q) ? 1f : 0f));
		rb.MovePosition(transform.position + transform.forward * forward + transform.right * right + Vector3.up * up);
	}
}
